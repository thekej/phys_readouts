...
def extract(args):
    ...
    # load model
    scorenet, config = load_model(args.model_path, device)
    ...
    if args.readout_type == 'SIMULATION':
        n_features = args.video_length // PRED_LENGTH
    ...
    #else:
    #    f4 = 1152 
    f1, f2, f3, f4 = 288, 2, 2, 768

    # set up new dataset
    f = h5py.File(args.save_file, "w")
    dset2 = f.create_dataset("label", (data_size,), dtype='f')
    dset4 = f.create_dataset("features", (data_size, n_features, f4, 1, 1), dtype='f')

    # extract features
    for i, (test_x, label) in enumerate(test_loader):
        input_frames = data_transform(config, test_x)
        #if not args.latent_type == 'middle_embeds':
        features_array = {'features': np.zeros((test_x.shape[0], n_features, f4, 1, 1)),
                             }
        #else:
        #    features_array = np.zeros((test_x.shape[0], n_features, f1, f2, f3))
        
        real, cond, cond_mask = conditioning_fn(config, input_frames[:, :13, :, :, :], num_frames_pred=config.data.num_frames,
        ...
        features_array['features'][:, 0, :, :, :] =  mid.numpy()
            
        for j in range(1, n_features):
            if args.readout_type == 'COMPLETE':
                real, cond, cond_mask = conditioning_fn(config, input_frames[:, 
            ...
            #if args.latent_type == 'middle_embeds':
            mid = nn.AdaptiveAvgPool3d((None, 1, 1))(mid.cpu())
            features_array['features'][:, j, :, :, :] =  mid.numpy()
            
        print('whole processing: ', time.time() - t)
        if pred.shape[0] == args.batch_size:
            dset2[i * args.batch_size:(i+1)*args.batch_size] = label
            dset4[i * args.batch_size: (i+1)*args.batch_size, :, :, :, :] = features_array['features']
        else:
            dset2[i * args.batch_size:] = label
            dset4[i * args.batch_size:, :, :, :, :] = features_array['features']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--video_length", type=int, default=13, help="Length of the video in frames")

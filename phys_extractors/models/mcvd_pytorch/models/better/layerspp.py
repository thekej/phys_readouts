...
class ResnetBlockDDPMppSPADE(nn.Module):
  ...
  def __init__(self, act, in_ch, out_ch=None, temb_dim=None, conv_shortcut=False, spade_dim=128,
    ...
    out_ch = out_ch if out_ch else in_ch
    self.GroupNorm_0 = MySPADE(norm_nc=(in_ch//n_frames if is3d else in_ch), 
                               label_nc=cond_ch, param_free_norm_type='group', 
                               act=act, conv=conv3x3_,
                               spade_dim=spade_dim, is3d=is3d, num_frames=n_frames, 
                               num_frames_cond=num_frames_cond, conv1x1=conv1x1_cond)
    self.Conv_0 = conv3x3_(in_ch, out_ch)
    if temb_dim is not None:
      self.Dense_0 = nn.Linear(temb_dim, out_ch)
      self.Dense_0.weight.data = default_init()(self.Dense_0.weight.data.shape)
  ...
  def forward(self, x, temb=None, cond=None):
    normed_x, gamma, beta = self.GroupNorm_0(x, cond)
    h = self.act(normed_x)
    h = self.Conv_0(h)
    if temb is not None:
      h += self.Dense_0(self.act(temb))[:, :, None, None]
    normed_h, gamma, beta = self.GroupNorm_1(h, cond)
    h = self.act(normed_h)
    h = self.Dropout_0(h)
    h = self.Conv_1(h)
    if x.shape[1] != self.out_ch:
...
class get_act_norm(nn.Module): # order is norm -> act
  def __init__(self, act, act_emb, norm, ch, emb_dim=None, spectral=False, is3d=False, n_frames=1,
    ...
    if norm == 'spade':
      self.Norm_0 = MySPADE(norm_nc=(ch // n_frames) if is3d else ch,
                            label_nc=cond_ch, param_free_norm_type='group', act=act, conv=cond_conv,
                            spade_dim=spade_dim, is3d=is3d, num_frames=n_frames, 
                            num_frames_cond=num_frames_cond,conv1x1=cond_conv1)
    else:
  ...
  def forward(self, x, emb=None, cond=None):
    if emb is not None:
      ...
      if self.norm == 'spade':
        emb_norm, gamma, beta = self.Norm_0(x, cond)
      ...
      if self.is3d:
        ...
        x = x.reshape(B, -1, N, H, W)
      if self.norm == 'spade':
        x, gamma, beta = self.Norm_0(x, cond)
...
class ResnetBlockBigGANppSPADE(nn.Module):
  ...
  def forward(self, x, temb=None, cond=None):
    h, _, _ = self.actnorm0(x, temb, cond)
    ...
    h = self.Conv_0(h)
    h, _, _ = self.actnorm1(h, temb, cond)
    h = self.Dropout_0(h)
    h = self.Conv_1(h)

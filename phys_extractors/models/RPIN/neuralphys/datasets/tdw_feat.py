...
class TDWPhys(Dataset):
    def __init__(
        ...
        self.preload_as_list = False #self.test
        ...
        self.data_path = [os.path.join(p, 'new_tfdata') if self.split == 'train' \
                else os.path.join(p, 'new_tfdata') for p in self.data_path]
        ...
        self.buffer_size = self.input_size + 1
        ...
        data_params['shift_selector'] = slice(*C.INPUT.TRAIN_SLICE) \
    ...
    def postprocess(self, data):
        ...
        scale_width = images.shape[-1] / self.input_width
        scale_height = images.shape[-1] / self.input_height
        ...
        bboxes = np.stack([
            bboxes[..., 0] / scale_height, # xmin
            bboxes[..., 1] / scale_width, # ymin
            bboxes[..., 2] / scale_height, # xmax
            bboxes[..., 3] / scale_width, # ymax
            bboxes[..., 4], # id
            ], axis = -1)
        rois = bboxes[..., :-1].copy()

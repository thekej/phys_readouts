...
from physion_loader import RPINDataset, RPINTrainDataset
...
def load_model(model, model_file):
    ...
    new_sd = OrderedDict()
    sd = torch.load(model_file)['model']
    for k, v in sd.items():
...
def extract_feat_step(model, data, device, num_rollouts,
    ...
    model.eval()
    with torch.no_grad():
        images, boxes, labels, data_last, ignore_mask, all_images, _ = data
        ...
        ignore_idx = ignore_mask.to(device)
        outputs = model(images, rois, coor_features, all_images=all_images,
                        num_rollouts=num_rollouts,
                        data_pred=data_last, phase='test', 
                        ignore_idx=ignore_idx)
    #print(outputs['encoded_states'].shape, outputs['rollout_states'].shape,outputs['full_outcome_states'].shape, outputs['bbox'].shape)
    input_states = torch.flatten(outputs['full_outcome_states'], 2).cpu().numpy()
    ...
    if save:
        images, boxes, labels, data_last, ignore_mask, all_images = data
        save_vis(data[0], data[1], data[3], 
             outputs['bbox'].cpu(), data[4], #stimulus_name, 
             output_dir)

    output = {
        'observed_full_outcome_states': input_states,
...
def main(args):
    ...
    print('load data')
    dataset = RPINTrainDataset(args.data_path)
    ...
    #torch.Size([64, 5, 10, 258]) torch.Size([64, 15, 10, 258]) torch.Size([64, 15, 10, 256]) torch.Size([64, 11, 10, 4])
    f = h5py.File(args.save_file, "w")
    dset1 = f.create_dataset("label", (data_size,), dtype='i')
    dset2 = f.create_dataset("observed", (data_size, 5, 2580), dtype='f')
    dset3 = f.create_dataset("observed_full_outcome", (data_size, 15, 2580), dtype='f')
    dset4 = f.create_dataset("simulation", (data_size, 15, 2580), dtype='f')
    ...
    for i, data in enumerate(tqdm.tqdm(loader)):        
        ...
        if (i+1)*args.batch_size < data_size:
            dset1[i*args.batch_size:(i+1)*args.batch_size] = data[6]
        else:
            dset1[i*args.batch_size:] = data[6]
        # input is (Bs, T, 3, H, W)
        #output = model(v_in)
        if (i+1)*args.batch_size < data_size:#(8, 4, 50, 16, 16) (8, 4, 50, 8, 8, 256) (8, 4, 5, 8, 8, 256)
            dset2[i*args.batch_size:(i+1)*args.batch_size] = output["observed_states"]#.detach().cpu().numpy()
            dset3[i*args.batch_size:(i+1)*args.batch_size] = output["observed_full_outcome_states"]#.detach().cpu().numpy()
            dset4[i*args.batch_size:(i+1)*args.batch_size] = output["simulated_states"]#.detach().cpu().numpy()
        else:
            dset2[i*args.batch_size:] = output["observed_states"]#.detach().cpu().numpy()
            dset3[i*args.batch_size:] = output["observed_full_outcome_states"]#.detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_rollouts', type=int, default=11)

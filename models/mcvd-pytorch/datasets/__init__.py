...
def inverse_data_transform(config, X, return_latents=False):
    if return_latents:
    # X is a tuple containing 4 values
        X, gammas, betas, mid_embeds = X
    X = X[-1].to('cpu')
    if hasattr(config, 'image_mean'):
    ...
    if config.data.logit_transform:
        ...
        X = (X + 1.) / 2.
    if return_latents:
        return torch.clamp(X, 0.0, 1.0), gammas, betas, mid_embeds

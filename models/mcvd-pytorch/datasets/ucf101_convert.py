...
def read_splits(splits_dir, split_idx, ucf_dir):
    ...
    # classes
    classes = {line.split(' ')[-1]: int(line.split(' ')[0])-1 for line in open(os.path.join(splits_dir, 'classInd_phys.txt'), 'r').read().splitlines()}
    classes_train = [0 for f in vids_train]#[classes[os.path.basename(os.path.dirname(f))] for f in vids_train]
    classes_test = [0 for f in vids_test]#[classes[os.path.basename(os.path.dirname(f))] for f in vids_test]

...
class NCSNpp(nn.Module):
  ...
  def forward(self, x, time_cond, cond=None, cond_mask=None):
    ...
    return h, None, None, middle_embed
...
class SPADE_NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    modules.append(layerspp.get_act_norm(act=act, act_emb=act, norm='spade', 
                                         ch=in_ch, is3d=self.is3d, n_frames=self.num_frames, num_frames_cond=num_frames_cond,
                                         cond_ch=num_frames_cond*channels, spade_dim=spade_dim, cond_conv=conv3x3,
  ...
  def forward(self, x, time_cond, cond=None, cond_mask=None):
    ...
    assert not hs
    # GroupNorm
    h, gamma, beta = modules[m_idx](h, cond=cond)
    m_idx += 1
    ...
    return h, gamma, beta, middle_embed

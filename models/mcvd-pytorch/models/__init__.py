...
def FPNDM_sampler(x_mod, scorenet, cond=None, final_only=False, denoise=True, subsample_steps=None,
    ...
    images, gamma_spades, beta_spades, middle_embeds = [], [], [], []
    ...
    for i, step in enumerate(steps):
        ...
        if not final_only:
            ...
            #gamma_spades.append(gamma_spade_embed.to('cpu'))
            #beta_spades.append(beta_spade_embed.to('cpu'))
            #middle_embeds.append(middle_embed.to('cpu'))

    if final_only:
        return x_mod.unsqueeze(0)#, gamma_spade_embed, beta_spade_embed, middle_embed
    else:
        return torch.stack(images)#, gamma_spades, beta_spades, middle_embeds
...
def ddim_sampler(x_mod, scorenet, cond=None, final_only=False, denoise=True, subsample_steps=None,
    ...
    for i, step in enumerate(steps):
        ...
        grad, gamma_spade_embed, beta_spade_embed, middle_embed = scorenet(x_mod, labels)
    ...
    if denoise: # x + batch_mul(std ** 2, score_fn(x, eps_t))
        ...
        x_mod = x_mod - (1 - alphas[-1]).sqrt() * scorenet(x_mod, last_noise)[0]
        if not final_only:
            images.append(x_mod.to('cpu'))
            gamma_spades.append(gamma_spade_embed.to('cpu'))
            beta_spades.append(beta_spade_embed.to('cpu'))
...
def anneal_Langevin_dynamics(x_mod, scorenet, cond=None, n_steps_each=200, step_lr=0.000008,
    ...
    if denoise:
        ...
        x_mod = x_mod - sigmas[-1] * scorenet(x_mod, last_noise)[0]
        if not final_only:
            images.append(x_mod.to('cpu'))
            gamma_spades.append(gamma_spade_embed.to('cpu'))
            beta_spades.append(beta_spade_embed.to('cpu'))
...
def sparse_anneal_Langevin_dynamics(x_mod_sparse, sparsity, scorenet, cond=None, n_steps_each=200, step_lr=0.000008,
    ...
    if denoise:
        last_noise = ((len(sigmas) - 1) * torch.ones(x_mod.shape[0], device=x_mod.device)).long()
        x_mod_sparse = x_mod_sparse - sigmas[-1] * sparsity * scorenet(x_mod, last_noise)[0]
        if not final_only:
            images.append(x_mod_sparse.to('cpu'))
            gamma_spades.append(gamma_spade_embed.to('cpu'))
            beta_spades.append(beta_spade_embed.to('cpu'))
    ...
    if final_only:
        return x_mod_sparse.unsqueeze(0), gamma_spade_embed, beta_spade_embed, middle_embed
    else:
        return torch.stack(images), gamma_spades, beta_spades, middle_embeds
...
def anneal_Langevin_dynamics_consistent(x_mod, scorenet, cond=None, n_steps_each=200, step_lr=0.000008,
    ...
    for c in range(consistent_L):
        ...
        grad, gamma_spade_embed, beta_spade_embed, middle_embed = scorenet(x_mod, used_sigmas, y_is_label=False)
    ...
    for c in range(consistent_L):
        ...
        if last_step:
            ...
            if denoise:
                last_noise = ((len(sigmas) - 1) * torch.ones(x_mod.shape[0], device=x_mod.device)).long()
                x_mod = x_mod + sigmas[-1] * scorenet(x_mod, last_noise)[0]
                x_mod_sparse = x_mod_sparse + sigmas[-1] * 1/sparsity * scorenet(x_mod, last_noise)[0]
                if not final_only:
                    images.append(x_mod.to('cpu'))
                    gamma_spades.append(gamma_spade_embed.to('cpu'))
                    beta_spades.append(beta_spade_embed.to('cpu'))

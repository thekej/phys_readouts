...
for lr in lr_list:
    ...
    for epoch_num in range(num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            feature = F.interpolate(seg_color.permute(0, 3, 1, 2).cuda(), size).permute(0, 2, 3, 1).float()  # [B, h, w, 3]
        ...
        if epoch_num % val_after == 0:
            mean_iou = compute_mean_iou_over_dataset(val_loader, decoder, size)
...
test_iou = compute_mean_iou_over_dataset(test_loader, decoder, size)
